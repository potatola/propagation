...
public abstract class SimuPropogation {
	protected int[] count = new int[720];
	...
	public SimuPropogation(List<NodeUnit> Nodes, int NodesLimit,
			double TimeLimit, double EnergyLimit, double theta) {
		this.Nodes = Nodes;
		...
		ActiveHeap = new PriorityQueue<ActiveHeapElement>(NodesLimit,
				acomparator);
		DeclineHeap = new PriorityQueue<DeclineHeapElement>(NodesLimit,
				dcomparator);
	}
	 ...
	 * 初始化，给定D和放一些节点入堆
	 */
	public void Initialization(int StartNodes[], String name) {
	...
	/*
	 * 终止条件 返回true继续运行 返回false停止。 静态时，dc为false，节点到限制且时间到限制后，才停止。输出两种模拟结果
	 * 动态时，三个条件限制,都不行了才会停止。 不过如果栈为空，则立刻停止（很少遇到这个情况）
	 */
	public boolean TerminateCondition() {
		...
		if (queueC && ActiveHeap.isEmpty()) {
			System.out.println("没有活节点啦。感染了" + NumOfActivenodes + "个节点，"
					+ SeenNodes.size() + "个节点看到，用时" + CurrentTime);
			queueC = false;
		}
	 ...
	 * 模拟跑起来~
	 */
	public void Run(int StartNodesID[], String name) {
		// 选节点
		// 初始化heap
		Initialization(StartNodesID, name);
		...
		try {
			os = new BufferedWriter(new FileWriter(new File("E:\\data_op\\"
					+ name)));
			for (int i = 0; i < 720; i++)
				os.write(count[i] + "\n");
		} catch (IOException e) {
